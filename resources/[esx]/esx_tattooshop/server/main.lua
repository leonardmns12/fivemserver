...
ESX.RegisterServerCallback('esx_tattooshop:purchaseTattoo', function(source, cb, tattooList, price, tattoo)
	...
	if xPlayer.getMoney() >= price then
		...
		TriggerClientEvent('mythic_notify:client:SendAlert', source, { type = 'success', text = 'Kamu telah membayar tato ini seharga Rp ' ..ESX.Math.GroupDigits(price), length = 2500, style = { ['background-color'] = '#2f5c73f', ['color'] = '#ffffff' } })
		cb(true)
	else
		local missingMoney = price - xPlayer.getMoney()
		--TriggerClientEvent('esx:showNotification', source, _U('not_enough_money', ESX.Math.GroupDigits(missingMoney)))
		TriggerClientEvent('mythic_notify:client:SendAlert', source, { type = 'error', text = 'Kamu tidak punya cukup uang untuk membeli tato ini! Uang kamu kurang Rp ' ..ESX.Math.GroupDigits(missingMoney), length = 2500, style = { ['background-color'] = '#2f5c73f', ['color'] = '#ffffff' } })
		cb(false)
	end
end)
