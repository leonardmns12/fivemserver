    ...
    TriggerEvent('esx_status:getStatus','stress',function(status)
      ...
      if status.val >= (self.GetSickAt * 10000) then
        ...
        if not self.IsSick then
          self.IsSick = true
          exports['mythic_notify']:SendAlert('error', 'You have become sick from too much stress. Try some relieving activities.')
        else
          ...
          elseif self.ExtremelySick then 
            ...
            exports['mythic_notify']:SendAlert('inform', 'You feel a little better. Keep up the calming activities.')
          end
        end
      elseif status.val <= (self.GetSickAt * 10000) then
        if self.IsSick then
          self.IsSick = false
          exports['mythic_notify']:SendAlert('inform', 'You feel more relaxed.')
