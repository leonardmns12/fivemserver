            ...
            if distance ~= -1 and distance <= 3.0 then
              if data2.current.value == 'cuff' then
                if Config.EnableItems then
                    ...
                    IsAbleToSearch = true
                    TriggerServerEvent('cuffServer', GetPlayerServerId(player))
                    -- TriggerEvent('cuffClient', closestID)
                end
              end
                ...
                if IsPedArmed(ped, 7) then
                    if IsAbleToSearch then
                        ...
                        elseif distance < 20 and distance > 2.0 then
                            exports['mythic_notify']:SendAlert('inform', 'Terlalu jauh')
                        else
                            exports['mythic_notify']:SendAlert('inform', 'Tidak ada orang disekitar')
                        end
                    else
                        exports['mythic_notify']:SendAlert('error', 'Player Tidak Dicuff')
                    
                    end
                else
                    exports['mythic_notify']:SendAlert('error', 'Kamu perlu mengeluarkan Senjata')
                end
              end
            else
                exports['mythic_notify']:SendAlert('inform', 'Tidak ada orang disekitar')
...
AddEventHandler('cuffClient', function()
    ...
    exports['mythic_notify']:SendAlert('error', 'Seseorang memborgol anda!')
...
AddEventHandler('unCuffClient', function()
	...
	SetCurrentPedWeapon(pP, GetHashKey('WEAPON_UNARMED'), true)
    exports['mythic_notify']:SendAlert('error', 'Borgol Dilepas!')
