...
function DrawTextOptions(carPos,currentName,currentDownpayment,currentCommission,swapCar,currentPrice,currentStock)
	if (PlayerData.job ~= nil and PlayerData.job.name == Config.CarDealerJobLabel) then
		...
		-- finance:
		DrawText3Ds(carPos[1],carPos[2],carPos[3] + 0.48, "~s~ Hubungi ~g~ Car Dealer ~s~ untuk tipe / jenis mobil lain")
		-- cost display:
		...
		-- buttons:
		DrawText3Ds(carPos[1],carPos[2],carPos[3] + 0.54, " ~g~[E]~s~ Beli | ~p~"..currentCommission.."%~s~ Pajak~s~")
		-- finance:
		DrawText3Ds(carPos[1],carPos[2],carPos[3] + 0.48, "~s~ Hubungi ~g~ Car Dealer ~s~ untuk tipe / jenis mobil lain")
		-- cost display:
		DrawText3Ds(carPos[1],carPos[2],carPos[3] + 0.42, "~y~"..Config.InterestRate.."%~s~ Interest Rate | ~o~$"..comma_value(math.floor((currentPrice + (currentPrice * (currentCommission / 100))) * ((Config.InterestRate/100)+1))).."~s~ Harga awal ")
	end
end
