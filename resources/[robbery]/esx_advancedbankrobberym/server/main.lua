					...
					for i=1, #xPlayers, 1 do
						...
						-- if xPlayer.job.name == 'police' then
							...
							TriggerClientEvent('chat:addMessage', -1, {
       						 template = '<div class="chat-message heist"><b>[NEWS] Telah terjadi heist di:</b> {0}</div>',
      						  args = { Bank["bankName"] }
							})
								
							TriggerClientEvent('mythic_notify:client:SendAlert', xPlayers[i], { type = 'error', text = 'Perampokan terjadi di '..Bank["bankName"]..'.', length = 3000, style = { ['background-color'] = '#2f5c73f', ['color'] = '#ffffff' } })
...
AddEventHandler("esx_advancedbankrobberym:endRobbery", function(bankId)
	...
	for i=1, #xPlayers, 1 do
 		local xPlayer = ESX.GetPlayerFromId(xPlayers[i])
		 if xPlayer.job.name == 'police' then
			TriggerClientEvent('mythic_notify:client:SendAlert', xPlayers[i], { type = 'inform', text = 'Perampokan di '..Bank["bankName"]..' telah dibatalkan.', length = 3000, style = { ['background-color'] = '#2f5c73f', ['color'] = '#ffffff' } })
