...
AddEventHandler('esx_holdupbank:plantedbomb', function(x,y,z,doortype)
	local coords = vector3(254.32739257813,225.49626159668,101.87580108643) -- fix for vectors
...
AddEventHandler('esx_holdupbank:opendoors', function(x,y,z,doortype)
	...
	local obs, distance = ESX.Game.GetClosestObject(coords,doortype) -- instant open for people already in site
	...
	while dooropen do
		Wait(2000)
		local obs, distance = ESX.Game.GetClosestObject(globalcoords, globalDoortype)
		SetEntityHeading(PlayerPedId(3), globalrotation)
	end
	end)
end)
		...
		for k,v in pairs(Banks)do
			...
			if(Vdist(pos.x, pos.y, pos.z, pos2.x, pos2.y, pos2.z) < 15.0)then
				if not holdingup then
					DrawMarker(21, v.position.x, v.position.y, v.position.z, 0, 0, 0, 0, 0, 0, 0.9, 0.5, 1.0, 1555, 0, 0,255, true, 0, 0,0)
		...
		for k,v in pairs(Banks)do
			...
			if(Vdist(pos.x, pos.y, pos.z, pos2.x, pos2.y, pos2.z) < 15.0)then
				if not hackholdingup then
					DrawMarker(21, v.hackposition.x, v.hackposition.y, v.hackposition.z, 0, 0, 0, 0, 0, 0, 0.9, 0.5, 1.0, 1555, 0, 0,255, true, 0, 0,0)
			...
			if (pos2 ~= nil) then
				if(Vdist(pos.x, pos.y, pos.z, pos2.x, pos2.y, pos2.z) < 15.0)then
					if not bombholdingup then
						DrawMarker(21, v.bombposition.x, v.bombposition.y, v.bombposition.z, 0, 0, 0, 0, 0, 0, 0.9, 0.5, 1.0, 1555, 0, 0,255, true, 0, 0,0)
